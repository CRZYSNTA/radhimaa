// sphere.frag - Holographic Fractured Sphere Fragment Shader
uniform vec3 uColor;
uniform float uTime;
...
void main() {
    ...
    // Fresnel rim effect (cinematic edge illumination)
    ...
    // Fractured digital noise mask (breaking sphere into futuristic segments)
    float n = noise(vUv * 16.0 + vec2(uTime * 0.05, 0.0));
    float digitalMask = step(0.42, n);

    // Combine layers: rim glow + grid + scan band + core luminescence
    float intensity = (fresnel * 1.8 + grid * 0.9 + scan * 1.5 + 0.15) * digitalMask;
    intensity += fresnel * 0.6; // keep outer rim visible even outside mask
    intensity *= (1.0 + uAudioLevel * 2.0);

    // Core color gradient with high-energy white/hot center highlights
    vec3 finalColor = uColor * intensity;
    vec3 hotGlow = mix(uColor, vec3(1.0, 0.95, 0.8), fresnel * 0.7);

    gl_FragColor = vec4(mix(finalColor, hotGlow, fresnel * 0.5), intensity * uOpacity);
}
